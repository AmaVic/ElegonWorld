...
public:
	...
	void OnDamage(Unit* attacker, Unit* /*victim*/, uint32& damage) {
		if (Player* player = attacker->ToPlayer()) {
			//Quêtes: 150000;300000;450000;1000000
    ...
    struct npc_training_dummyAI : ScriptedAI
    {
        ...
        void DamageTaken(Unit* doneBy, uint32& damage) override
        {
			...
			if (Player* player = doneBy->ToPlayer()) {
				//Quêtes: 150000;300000;450000;1000000
				if (damage >= 150000 && damage < 300000000000)
				{
					if (player->GetQuestStatus(200233) == QUEST_STATUS_INCOMPLETE)
					{
						player->KilledMonsterCredit(3002);
					}
				}
				if (damage >= 300000 && damage < 450000000000)
				{
					if (player->GetQuestStatus(200234) == QUEST_STATUS_INCOMPLETE)
					{
						player->KilledMonsterCredit(3002);
					}
				}
				if (damage >= 450000 && damage < 1000000000000) {
					if (player->GetQuestStatus(200235) == QUEST_STATUS_INCOMPLETE)
					{
						player->KilledMonsterCredit(3002);
					}
				}
				if (damage >= 1000000) {
					if (player->GetQuestStatus(200236) == QUEST_STATUS_INCOMPLETE)
					{
						player->KilledMonsterCredit(3002);
					}
				}
