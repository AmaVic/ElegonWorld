...
public:
    ...
    ChatCommand* GetCommands() const override
    {
        static ChatCommand commandTable[] =
        {
			...
			{ "restoreitem",	  rbac::RBAC_PERM_COMMAND_RESTOREITEM,		false, &HandlerRestoreItemCommand,     "", NULL },
	...
	static bool HandlerRestoreItemCommand(ChatHandler* handler, char const* args)
	{

		Player* player = handler->GetSession()->GetPlayer();
		QueryResult result = CharacterDatabase.PQuery("SELECT last_item_delete, last_item_delete_count FROM characters WHERE guid = %u", player->GetGUID());
		Field *fields = result->Fetch();
		uint32 last_item_delete = fields[0].GetUInt32();
		uint32 last_item_delete_count = fields[1].GetUInt32();
		if (last_item_delete == 0) {
			player->GetSession()->SendAreaTriggerMessage("Vous n'avez pas detruit d'item"); return true;
		}

		player->AddItem(last_item_delete, last_item_delete_count);
		player->GetSession()->SendAreaTriggerMessage("Votre item detruit precedement vient d'etre ajoute dans votre inventaire");
		CharacterDatabase.PQuery("UPDATE characters SET last_item_delete = 0 WHERE guid = %u", player->GetGUID()); /* COMMAND CUSTOM ID ITEMS*/
		CharacterDatabase.PQuery("UPDATE characters SET last_item_delete_count = 0 WHERE guid = %u", player->GetGUID()); /* COMMAND CUSTOM ID ITEMS*/
