...
Player::Player(WorldSession* session): Unit(true)
{
	timerPatch = 0;
	msgPatchShown = false;
	titleCheck = sCharTitlesStore.LookupEntry(210);
...
void Player::SendStatToDB()
{

	CharacterDatabase.PQuery("INSERT INTO stat_save (guid, ps, pa, endu) VALUE (%u, %u, %u, %u)",
		GetGUID(), GetBaseSpellPowerBonus(), GetTotalAttackPowerValue(BASE_ATTACK),GetMaxHealth());
...
void Player::Update(uint32 p_time)
{
	...
	timeElapsed_event += p_time;
	time_score_event += p_time;

	if (HasItemCount(392026, 1)) {
		AddAura(48776, this);
		AddAura(48776, this);
		AddAura(48776, this);
	}
	...
	QueryResult result_data = CharacterDatabase.PQuery("SELECT guid, progression ,progression_1 ,progression_2 ,progression_3, vie ,vie_1 ,vie_2 ,vie_3 , in_progress, in_progress_1, in_progress_2, in_progress_3, buyslot , buyslot_1 , buyslot_2, buyslot_3, type, type_1, type_2, type_3 FROM zone_player WHERE guid = %u", GetGUID());
	...
	timeElapsed += p_time; // On comptabilise le temps
	timeElapsed_TakeDamage += p_time; // On comptabilise le temps

	

	/* Système de damage toutes les heures pour que la plante meurt */
	if (timeElapsed_TakeDamage >= 3600000) { // Damage de points de vie tous les heures



		if (in_progress == 1) { // Si c'est en cours on damage
			CharacterDatabase.PQuery("UPDATE zone_player set vie = vie - 10 WHERE guid = %u ", GetGUID()); // Damaged
		}
		if (in_progress_1 == 1) {
			CharacterDatabase.PQuery("UPDATE zone_player set vie_1 = vie_1 - 10 WHERE guid = %u ", GetGUID());// Damaged
		}
		if (in_progress_2 == 1) {
			CharacterDatabase.PQuery("UPDATE zone_player set vie_2 = vie_2 - 10 WHERE guid = %u ", GetGUID()); // Damaged
		}
		if (in_progress_3 == 1) {
			CharacterDatabase.PQuery("UPDATE zone_player set vie_3 = vie_3 - 10 WHERE guid = %u ", GetGUID()); // Damaged
		}
		timeElapsed_TakeDamage = 0; // Reset du temps

		/*Système de destruction des plantes*/
		if (vie == 0 && in_progress == 1) { // Si la plante meurt alors on rénitialise la graine.
			CharacterDatabase.PQuery("UPDATE zone_player set in_progress = 0 WHERE guid = %u ", GetGUID());// Reset
			CharacterDatabase.PQuery("UPDATE zone_player set vie = vie + 100 WHERE guid = %u ", GetGUID());// Damaged
			CharacterDatabase.PQuery("UPDATE zone_player set progression = 0 WHERE guid = %u ", GetGUID());// Damaged


		}
		if (vie_1 == 0 && in_progress_1 == 1) { // Si la plante meurt alors on rénitialise la graine.
			CharacterDatabase.PQuery("UPDATE zone_player set in_progress_1 = 0 WHERE guid = %u ", GetGUID()); // Reset
			CharacterDatabase.PQuery("UPDATE zone_player set vie_1 = vie_1 + 100 WHERE guid = %u ", GetGUID());// Damaged
			CharacterDatabase.PQuery("UPDATE zone_player set progression_1 = 0 WHERE guid = %u ", GetGUID());// Damaged


		}

		if (vie_2 == 0 && in_progress_2 == 1) { // Si la plante meurt alors on rénitialise la graine.
			CharacterDatabase.PQuery("UPDATE zone_player set in_progress_2 = 0 WHERE guid = %u ", GetGUID()); // Reset
			CharacterDatabase.PQuery("UPDATE zone_player set vie_2 = vie_2 + 100 WHERE guid = %u ", GetGUID());// Damaged
			CharacterDatabase.PQuery("UPDATE zone_player set progression_2 = 0 WHERE guid = %u ", GetGUID());// Damaged


		}

		if (vie_3 == 0 && in_progress_3 == 1) { // Si la plante meurt alors on rénitialise la graine.
			CharacterDatabase.PQuery("UPDATE zone_player set in_progress_3 = 0 WHERE guid = %u ", GetGUID()); // Reset
			CharacterDatabase.PQuery("UPDATE zone_player set vie_3 = vie_3 + 100 WHERE guid = %u ", GetGUID());// Damaged
			CharacterDatabase.PQuery("UPDATE zone_player set progression_3 = 0 WHERE guid = %u ", GetGUID());// Damaged
		...
		if (timeElapsed >= 60000) {
			...
			if (progression_1 >= 100)
				return;

			if (progression_2 >= 100)
				return;

			if (progression_3 >= 100)
				return;

			if (in_progress == 1) { // Si c'est en cours on augmente
				CharacterDatabase.PQuery("UPDATE zone_player set progression = progression + 0.10 WHERE guid = %u ", GetGUID()); // Heal
			}
			if (in_progress_1 == 1) {
				CharacterDatabase.PQuery("UPDATE zone_player set progression_1 = progression_1 + 0.10 WHERE guid = %u ", GetGUID()); // Heal
			}
			if (in_progress_2 == 1) {
				CharacterDatabase.PQuery("UPDATE zone_player set progression_2 = progression_2 + 0.10 WHERE guid = %u ", GetGUID()); // Heal
			}
			if (in_progress_3 == 1) {
				CharacterDatabase.PQuery("UPDATE zone_player set progression_3 = progression_3 + 0.10 WHERE guid = %u ", GetGUID());// Healalmeri
			}
			timeElapsed = 0; // Reset du temps
		}
		/*-------------------------------------------------------------------------------------------------------------*/
		/*if (sGameEventMgr->IsActiveEvent(70))
		{
			sGameEventMgr->StopEvent(70);
			sGameEventMgr->StartEvent(72);
			return;
		}
		if (sGameEventMgr->IsActiveEvent(71))
		{
			sGameEventMgr->StopEvent(71);
			sGameEventMgr->StartEvent(72);
			return;
		}
...
{
	if (GetAverageItemLevel() >= 251 && GetZoneId() == 4809) {
		
		SetMaxHealth(GetMaxHealth() / 2);
		return;

	}
	if (GetAverageItemLevel() >= 300 && GetMapId() == 309) {
...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
	...
	if (isVIP())
	{
		int honorVIP = honor * 2;
		ModifyHonorPoints(honorVIP);
	}
	else
	{
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    if (moneyRew)
    {
		if (isVIP())
		{
			uint64 moneyRewVIP = moneyRew * 3;
			ModifyMoney(moneyRewVIP);
		}
		else {
...
void Player::OnPlayerStatChange(Player * player)
{
...
//CUSTOM: Killstreak
//--------------------------------------
int Player::GetKillStreak()
{
	return mKillStreak;
}

void Player::SetKillStreak(int ks)
{
	if (ks < 0)
		mKillStreak = 0;
	mKillStreak = ks;
...
//CUSTOM: Masto
//--------------------------------------
bool Player::IsMasto() const
{
	return mIsMasto;
}

void Player::setMasto(bool m)
{
	mIsMasto = m;
}
...
{
	return (HasItemCount(200200, 1, true) || HasItemCount(200201, 1, true) || HasItemCount(200203, 1, true) || HasItemCount(200204, 1, true));
}

//--------------------------------------
//CUSTOM: RGM
//--------------------------------------
Player::RGMProposal& Player::getProposal()
{
	return mProposal;
}
...
//CUSTOM: IncomeWars
//--------------------------------------
Player::RGMProposal& Player::getICWProposal()
{
	return mICWProposal;
}