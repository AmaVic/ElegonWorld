...
class Player : public Unit, public GridObject<Player>
{
	...
	public: //RGM: inner struct CUSTOM
	...
	struct RGMProposal
	{
		...
		//---------------------------
		RGMProposal() : inProgress(false), answer(false), elapsedTimeSinceProposal(0), hasAnswered(false) {}
    ...
    public:
		...
		void OnPlayerStatChange(Player * player);
		void SendStatToDB();
		...
		//---------------------------------------------------------
		int GetKillStreak();
		void SetKillStreak(int ks);

		bool IsMasto() const;
		void setMasto(bool m);

		RGMProposal& getProposal();
		RGMProposal& getICWProposal(); //IncomeWars

		uint32 timerPatch;
		bool msgPatchShown;
		CharTitlesEntry const* titleCheck;
    ...
    private:
		...
		int mKillStreak;
		uint64 timeElapsed;
		uint64 timeElapsed_TakeDamage;
		uint64 timeElapsed_event;
		uint64 time_score_event;
		uint64 timer_comptabilisation;
		bool isEventStarted = false;
		bool isMsgStarter = false;
		...
		//CUSTOM: RGM
		//---------------------------------
		RGMProposal mProposal;

		//---------------------------------
		//CUSTOM: IncomeWars
		//---------------------------------
		RGMProposal mICWProposal;
