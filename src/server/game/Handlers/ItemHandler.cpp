...
void WorldSession::HandleDestroyItemOpcode(WorldPacket& recvData)
{
	...
	if (count)
	{
		...
		uint32 i_count = count;
	CharacterDatabase.PQuery("UPDATE characters SET last_item_delete = %u, last_item_delete_count = %u WHERE guid = %u", pItem->GetEntry(), pItem->GetCount(), _player->GetGUID()); /* COMMAND CUSTOM */
