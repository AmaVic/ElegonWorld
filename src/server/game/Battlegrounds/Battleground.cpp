...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    if (isArena() && isRated())
    {
		...
		{
			if (winnerArenaTeam->IsMember(itr->first))
			{
				Player* p = sObjectAccessor->FindPlayer(itr->first);
				if (p)
				{
					p->KilledMonsterCredit(3009);
				}
			}
		}
		//--------------------------------------------
	...
	//Enregistrement du score: CUSTOM
	uint8 aliveWinners2 = GetAlivePlayersCountByTeam(winner);
	for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
	{
		Player* player = _GetPlayer(itr, "EndBattleground");
		if (!player)
			continue;
		BGScore score(player->GetGUID(), GetTypeID());
		score.load();
		score.update(winner == GetPlayerTeam(player->GetGUID()));
	}
...
void Battleground::RemovePlayerAtLeave(uint64 guid, bool Transport, bool SendPacket)
{
	...
	//CUSTOM: on enregistre la défaite:
	BGScore leaverScore(guid, GetTypeID());
	leaverScore.update(false);
